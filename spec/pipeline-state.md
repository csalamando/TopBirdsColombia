...
- **Sprint 14 (preparación) 2026-09-05**: diagrama de despliegue regenerado desde `railway.toml` (`iac_to_diagram.py --railway`, check sin drift) y **aceptado con recibo GATE-3** (`spec/receipts/despliegue.drawio.receipt.json`, rol cloud-engineer, aprobación del usuario). `railway.toml` validado (TOML) y empujado a `origin/main`; imagen Docker `topbirds:latest` verificada localmente (`/health` ok). **Pendiente para GATE 3 completo**: deploy real en Railway (CLI instalado pero sesión expirada; el usuario debe ejecutar `railway login`, `railway link` y `railway up`), validación de staging y prueba de rollback (re-deploy de commit anterior).
