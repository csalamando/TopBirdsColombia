...
| S19-AR-02 | [MOVIDA AL ARNÉS — es feature de harness-sdlc, no de este proyecto] Portal con auditoría y trazabilidad: revisar `spec/portal/` + `spec/dashboard.html` para explotar la memoria de auditoría (`spec/audit/events.jsonl`) adoptada en v2.22 — página de auditoría con línea de tiempo de eventos (emit/revoke/arch_lint/contract_diff) con aprobador y harness_version por evento, contador real de recibos revocados (hoy la métrica existe pero no se muestra), estado de gates por artefacto (de `gate_verify`), e indicador de drift de scripts vendorados (de `harness_doctor --check-vendored`). Criterios: (a) la página de auditoría deriva 100% de `events.jsonl` (nada narrado a mano); (b) el contador de revocados coincide con `audit_verify.py`/métricas derivadas; (c) cada vista nueva queda registrada en `registry.json` vía su generador; (d) `harness_graph.py --proyecto . --check` sigue en verde en CI tras el cambio. Garantía de actualización ya vigente: `ci-spec-governance.yml` corre el drift check del portal en cada push que toca `spec/**` — esta HU solo añade fuentes de datos, no nuevos mecanismos de frescura. Origen: verificación del proyecto contra arnés v2.22. SEGUIMIENTO en harness-sdlc: docs/propuesta-portal-auditoria-v2.23.md (reemplaza esta HU). Este proyecto solo adoptará la feature al actualizar el arnés vendored a v2.23 | EP-04 | Should have | - | Movida |
