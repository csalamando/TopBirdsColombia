...
def parse_railway(path):
    """→ [(group, rtype, name)] derivado de railway.toml (config-as-code).

    El nombre del servicio no vive en railway.toml (es por-servicio); se
    etiqueta como 'web-service'. El volumen de datos se deriva de
    DATABASE_URL (basename) cuando está definido.
    """
    import tomllib
    ...
    nodes = [("railway", "railway_web_service", "web-service")]
    if build.get("dockerfilePath"):
        nodes.append(("railway", "dockerfile", build["dockerfilePath"]))
    if deploy.get("healthcheckPath"):
        nodes.append(("railway", "healthcheck", deploy["healthcheckPath"]))
    env = deploy.get("envVars", {})
    if env.get("DATABASE_URL"):
        nodes.append(("railway-data", "sqlite_volume",
                      os.path.basename(env["DATABASE_URL"])))
    return nodes
